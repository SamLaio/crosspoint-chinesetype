...
// ========== 复用：中英文映射函数（和SettingsActivity保持一致） ==========
static const char* getChineseName(const char* englishName) {
    ...
    if (strcmp(englishName, "Toggle") == 0) return "切换";
    ...
    if (strcmp(englishName, "Inverted") == 0) return "反转竖屏";
...
void CategorySettingsActivity::render() const {
  ...
  // ========== 核心修改1：分类标题显示中文（如 Display → 显示） ==========
  renderer.drawCenteredText(UI_12_FONT_ID, 15, getChineseName(categoryName), true, EpdFontFamily::BOLD);
  ...
  for (int i = 0; i < settingsCount; i++) {
    ...
    // ========== 核心修改2：设置项名称显示中文（如 Sleep Screen → 休眠屏幕） ==========
    renderer.drawText(UI_10_FONT_ID, 20, settingY, getChineseName(settingsList[i].name), !isSelected);

    // Draw value based on setting type
    std::string valueText;
    if (settingsList[i].type == SettingType::TOGGLE && settingsList[i].valuePtr != nullptr) {
      const bool value = SETTINGS.*(settingsList[i].valuePtr);
      // ========== 核心修改3：Toggle值显示中文（ON/OFF → 开启/关闭） ==========
      valueText = value ? getChineseName("ON") : getChineseName("OFF");
    } else if (settingsList[i].type == SettingType::ENUM && settingsList[i].valuePtr != nullptr) {
      const uint8_t value = SETTINGS.*(settingsList[i].valuePtr);
      // ========== 核心修改4：枚举值显示中文（如 Dark → 深色） ==========
      valueText = getChineseName(settingsList[i].enumValues[value].c_str());
  ...
  // ========== 核心修改5：按钮提示显示中文（« Back → « 返回，Toggle → 切换） ==========
  const auto labels = mappedInput.mapLabels(getChineseName("« Back"), getChineseName("Toggle"), "", "");
