...
void ClearCacheActivity::render() {
  ...
  if (state == WARNING) {
    renderer.drawCenteredText(UI_10_FONT_ID, pageHeight / 2 - 60, "此操作将清除所有书籍缓存数据。", true);
    renderer.drawCenteredText(UI_10_FONT_ID, pageHeight / 2 - 30, "所有阅读进度都将丢失！", true,
                              EpdFontFamily::BOLD);
    renderer.drawCenteredText(UI_10_FONT_ID, pageHeight / 2 + 10, "书籍再次打开时需要重新建立索引!!!", true);
    //renderer.drawCenteredText(UI_10_FONT_ID, pageHeight / 2 + 30, "。", true);

    const auto labels = mappedInput.mapLabels("返回", "确认清除", "", "");
  ...
  if (state == SUCCESS) {
    renderer.drawCenteredText(UI_10_FONT_ID, pageHeight / 2 - 20, "已清除", true, EpdFontFamily::BOLD);
    String resultText = String(clearedCount) + " 本被删除";
    if (failedCount > 0) {
      resultText += ", " + String(failedCount) + " 失败";
    }
  ...
  if (state == FAILED) {
    renderer.drawCenteredText(UI_10_FONT_ID, pageHeight / 2 - 20, "清除缓存失败", true, EpdFontFamily::BOLD);
    renderer.drawCenteredText(UI_10_FONT_ID, pageHeight / 2 + 10, "查看细节");
