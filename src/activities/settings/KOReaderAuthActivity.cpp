...
void KOReaderAuthActivity::onWifiSelectionComplete(const bool success) {
  ...
  statusMessage = getChineseName("Authenticating...");
  xSemaphoreGive(renderingMutex);
  updateRequired = true;
...
void KOReaderAuthActivity::performAuthentication() {
  ...
  if (result == KOReaderSyncClient::OK) {
    state = SUCCESS;
    statusMessage = getChineseName("Successfully authenticated!");
  } else {
    state = FAILED;
...
void KOReaderAuthActivity::render() {
  ...
  if (state == SUCCESS) {
    renderer.drawCenteredText(UI_10_FONT_ID, 280, getChineseName("Success!"), true, EpdFontFamily::BOLD);
    renderer.drawCenteredText(UI_10_FONT_ID, 320, getChineseName("KOReader sync is ready to use"));

    const auto labels = mappedInput.mapLabels(getChineseName("Done"), "", "", "");
