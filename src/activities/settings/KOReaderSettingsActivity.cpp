...
const char* menuNames[MENU_ITEMS] = {"用户名", "密码", "同步服务器地址", "文档匹配方式", "验证身份"};
}  // namespace
...
void KOReaderSettingsActivity::handleSelection() {
  ...
  if (selectedIndex == 0) {
    ...
    enterNewActivity(new KeyboardEntryActivity(
        renderer, mappedInput, "KOReader 用户名", KOREADER_STORE.getUsername(), 10,
        64,     // maxLength
        false,  // not password
    ...
    exitActivity();
    enterNewActivity(new KeyboardEntryActivity(
        renderer, mappedInput, "KOReader 密码", KOREADER_STORE.getPassword(), 10,
        64,     // maxLength
    ...
    exitActivity();
    enterNewActivity(new KeyboardEntryActivity(
        renderer, mappedInput, "同步服务器地址", prefillUrl, 10,
...
void KOReaderSettingsActivity::render() {
  ...
  // Draw header
  renderer.drawCenteredText(UI_12_FONT_ID, 15, "KOReader 设置", true, EpdFontFamily::BOLD);
  ...
  for (int i = 0; i < MENU_ITEMS; i++) {
    ...
    const char* status = "";
    if (i == 0) {
      status = KOREADER_STORE.getUsername().empty() ? "[无记录]" : "[已保存]";
    } else if (i == 1) {
      status = KOREADER_STORE.getPassword().empty() ? "[无记录]" : "[已保存]";
    } else if (i == 2) {
      status = KOREADER_STORE.getServerUrl().empty() ? "[无记录]" : "[已保存]";
    } else if (i == 3) {
      status = KOREADER_STORE.getMatchMethod() == DocumentMatchMethod::FILENAME ? "[文件名]" : "[二进制匹配]";
    } else if (i == 4) {
      status = KOREADER_STORE.hasCredentials() ? "" : "[请先配置认证凭据]";
    }
