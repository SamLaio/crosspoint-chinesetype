...
void HomeActivity::renderRecentBooks(const Rect& rect) const {
  ...
  for (int col = 0; col < kRecentColumns; col++) {
    ...
    const auto titleLines = wrapTextToLines(renderer, book.title.c_str(), UI_10_FONT_ID, tileWidth, 2);
    int titleY = progressY + progressHeight + progressGap;
    for (const auto& titleLine : titleLines) {
      renderer.drawText(UI_10_FONT_ID, tileX, titleY, titleLine.c_str());
      titleY += titleHeight;
    }
  }
}
...
void HomeActivity::renderFileEntries(const Rect& rect) const {
  ...
  for (int i = 0; i < fileItemsPerPage; i++) {
    ...
    const auto lines = wrapTextToLines(renderer, label.c_str(), UI_10_FONT_ID, tileWidth - 12, 2);
    ...
    for (const auto& line : lines) {
      const int textX = tileX + (tileWidth - renderer.getTextWidth(UI_10_FONT_ID, line.c_str())) / 2;
      renderer.drawText(UI_10_FONT_ID, textX, textY, line.c_str(), !selected);
      textY += lineHeight;
    }
  }
}
