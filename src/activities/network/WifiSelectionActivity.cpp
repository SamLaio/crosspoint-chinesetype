...
void WifiSelectionActivity::selectNetwork(const int index) {
  ...
  if (selectedRequiresPassword) {
    ...
    enterNewActivity(new KeyboardEntryActivity(
        renderer, mappedInput, "请输入wifi密码 确认后点击OK",
        "",     // No initial text
        50,     // Y position
...
void WifiSelectionActivity::renderNetworkList() const {
  ...
  // Draw header
  renderer.drawCenteredText(UI_12_FONT_ID, 15, "wifi 网络", true, EpdFontFamily::BOLD);

  if (networks.empty()) {
    // No networks found or scan failed
    const auto height = renderer.getLineHeight(UI_10_FONT_ID);
    const auto top = (pageHeight - height) / 2;
    renderer.drawCenteredText(UI_10_FONT_ID, top, "找不到可用网络");
    renderer.drawCenteredText(SMALL_FONT_ID, top + height + 10, "按确认键重新扫描");
  } else {
    ...
    // Show network count
    char countStr[32];
    snprintf(countStr, sizeof(countStr), "找到wifi数目：%zu ", networks.size());
  ...
  // Draw help text
  renderer.drawText(SMALL_FONT_ID, 20, pageHeight - 75, "* = 已加密 | + = 已保存");
...
void WifiSelectionActivity::renderConnecting() const {
  ...
  if (state == WifiSelectionState::SCANNING) {
    renderer.drawCenteredText(UI_10_FONT_ID, top, "扫描中...");
  } else {
    renderer.drawCenteredText(UI_12_FONT_ID, top - 40, "连接中...", true, EpdFontFamily::BOLD);
...
void WifiSelectionActivity::renderConnected() const {
  ...
  renderer.drawCenteredText(UI_12_FONT_ID, top - 30, "连接成功!", true, EpdFontFamily::BOLD);
  ...
  const auto labels = mappedInput.mapLabels("", "继续", "", "");
  renderer.drawButtonHints(UI_10_FONT_ID, labels.btn1, labels.btn2, labels.btn3, labels.btn4);
}
...
void WifiSelectionActivity::renderSavePrompt() const {
  ...
  renderer.drawCenteredText(UI_12_FONT_ID, top - 40, "连接成功!", true, EpdFontFamily::BOLD);
  ...
  renderer.drawCenteredText(UI_10_FONT_ID, top + 40, "是否保存密码?");
  ...
  // Use centralized button hints
  const auto labels = mappedInput.mapLabels("跳过", "选择", "<", ">");
  renderer.drawButtonHints(UI_10_FONT_ID, labels.btn1, labels.btn2, labels.btn3, labels.btn4);
}
...
void WifiSelectionActivity::renderForgetPrompt() const {
  ...
  renderer.drawCenteredText(UI_12_FONT_ID, top - 40, "删除网络?", true, EpdFontFamily::BOLD);
  ...
  renderer.drawCenteredText(UI_10_FONT_ID, top + 40, "删除已保存密码");
  ...
  // Draw "Cancel" button
  if (forgetPromptSelection == 0) {
    renderer.drawText(UI_10_FONT_ID, startX, buttonY, "[取消]");
  } else {
    renderer.drawText(UI_10_FONT_ID, startX + 4, buttonY, "取消");
  }

  // Draw "Forget network" button
  if (forgetPromptSelection == 1) {
    renderer.drawText(UI_10_FONT_ID, startX + buttonWidth + buttonSpacing, buttonY, "[删除网络]");
  } else {
    renderer.drawText(UI_10_FONT_ID, startX + buttonWidth + buttonSpacing + 4, buttonY, "删除网络");
  }
