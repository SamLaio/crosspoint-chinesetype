...
// Helper to load a single font file — intervals NOT loaded into RAM
CustomEpdFont* loadFontFile(const String& path) {
  Serial.printf("[FontMgr] Loading metadata for: %s\n", path.c_str());
  FsFile f;
  ...
  uint32_t intervalCount, glyphCount, offsetIntervals, offsetGlyphs, offsetBitmaps;
  ...
  if (version == 1) {
    ...
    height = buf[3];
    Serial.printf("[FontMgr] Actual intervalCount from file: %u\n", intervalCount);
  } else {
  ...
  f.close(); // 👈 关闭文件，不再读 intervals！

  // Create EpdFontData — but DO NOT load intervals
  ...
  fontData->intervals = nullptr;      // ←←← 关键：设为 nullptr
  ...
  // Pass all needed info to CustomEpdFont
  return new CustomEpdFont(
      path,
      fontData,
      offsetIntervals,
      offsetGlyphs,
