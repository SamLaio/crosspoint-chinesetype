...
書名、OPDS 書目、Wi-Fi 名稱、檔名、章節名，以及其他來自外部的文字，會使用完整的 NotoSans 12 字型顯示，避免被 UI 子集字型限制而缺字。
...
藍牙設定可掃描並連線支援的 Bluetooth HID 裝置，例如外接翻頁鍵、鍵盤或遙控器。裝置名稱、訊號與連線狀態等外部文字會使用完整 NotoSans UI 字型顯示。
...
內建閱讀字型選項包含 Bookerly、NotoSans 與 OpenDyslexic 的字型 id。在目前版本中，編入韌體的完整 CJK 閱讀字型為 NotoSans；固定 UI 標籤則使用由 `LanguageMapper` 文字自動產生的較小子集字型。
...
建置時會自動從 `LanguageMapper` 重新產生 UI 字型子集，保留繁體中文、簡體中文與英文介面固定文字，同時降低韌體大小。

凡是來自韌體外部的文字，例如書名、作者、檔名、OPDS metadata、Wi-Fi SSID、URL、使用者名稱、章節名與進度訊息，都改用完整 NotoSans UI 字型，不依賴 UI 子集字型。
