...
bool Xtc::generateCoverBmp() const {
  ...
  // 計算需要正常渲染的行範圍（前 height - fillWhiteHeight 行）
  ...
  int32_t height = -static_cast<int32_t>(pageInfo.height); // 高度不變，保持原尺寸
  ...
  if (is2BitMode) {
    uint8_t palette2Bit[4][4] = {
      {0x00, 0x00, 0x00, 0x00}, // 0: 純黑
      {0x55, 0x55, 0x55, 0x00}, // 1: 深灰
      {0xAA, 0xAA, 0xAA, 0x00}, // 2: 淺灰
      {0xFF, 0xFF, 0xFF, 0x00}  // 3: 純白
    };
  ...
  if (is2BitMode) {
    ...
    for (uint16_t y = 0; y < pageInfo.height; y++) {
      ...
      if (y < normalRowEnd) {
        ...
        // 最後20行：強制填充白色（2bit白色值=3）
        for (uint16_t x = 0; x < pageInfo.width; x++) {
          const size_t bytePos = x / 4;
          const size_t bitPos = 6 - ((x % 4) * 2);
          rowBuffer[bytePos] &= ~(0b11 << bitPos);
          rowBuffer[bytePos] |= (3 << bitPos); // 3 = 2bit純白
        }
      }
    ...
    for (uint16_t y = 0; y < pageInfo.height; y++) {
      ...
      if (y < normalRowEnd) {
        ...
        for (size_t i = 0; i < srcRowSize; i++) {
          rowBuffer[i] = ~rowBuffer[i]; // 色反修復
        }
      } else {
        // 最後20行：強制填充白色（1bit白色值=1，位元組全1）
        memset(rowBuffer, 0xFF, srcRowSize); // 0xFF = 所有位都是1（純白）
      }
...
bool Xtc::scaleCoverToThumb(int height) const {
  ...
  Serial.printf("[%lu] [XTC] 從cover.bmp縮放到thumb: %dx%d -> %dx%d\n", millis(), coverW, coverH, outW, outH);
  return true;
}

//使用jpegtobmpconverter抖點,效果更好
