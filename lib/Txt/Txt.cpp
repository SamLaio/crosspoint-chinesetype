...
// Txt.cpp 实现
size_t Txt::getChapterEndOffsetByIndex(int chapterIndex) const {
  // 1. 先确定章节所属批次
  int batchStart = (chapterIndex / 25) * 25;
  int idxInBatch = chapterIndex % 25;

  // 2. 确保该批次已解析（无则解析）
  Txt* constThis = const_cast<Txt*>(this); // 临时解除const，适配现有parse逻辑
  constThis->parseChapterIndexAndOffset(batchStart);

  // 3. 校验索引有效性
  if (idxInBatch >= chapterActualCount) {
    return getFileSize() - 1; // 超出范围则取文件末尾
  }

  // 4. 计算结束偏移：下一章起始-1，无下一章则取文件末尾
  if (idxInBatch + 1 < chapterActualCount) {
    return chapterDataList[idxInBatch + 1].byteOffset - 1;
  } else {
    // 批次内最后一章，检查下一个批次是否有章节
    int nextBatchStart = batchStart + 25;
    constThis->parseChapterIndexAndOffset(nextBatchStart);
    if (chapterActualCount > 0) {
      return chapterDataList[0].byteOffset - 1;
    } else {
