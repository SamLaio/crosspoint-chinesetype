...
// 在文件的其他辅助函数附近添加
bool ChapterHtmlSlimParser::isEnglishPunctuation(char c) {
    return c == '.' || c == ',' || c == '!' || c == '?' || c == ';' || c == ':' || 
           c == '(' || c == ')' || c == '[' || c == ']' || c == '{' || c == '}' ||
           c == '"' || c == '\'' || c == '-' || c == '~' || c == '`' || c == '@' ||
           c == '#' || c == '$' || c == '%' || c == '^' || c == '&' || c == '*' ||
...
void XMLCALL ChapterHtmlSlimParser::characterData(void* userData, const XML_Char* s, const int len) {
  ...
  while (i < len) {
    ...
    // 检查是否为空白字符
    if ((s[i] & 0x80) == 0 && isWhitespace(s[i])) {
      // ASCII空白字符：如果有缓冲区内容，输出它
      if (self->partWordBufferIndex > 0) {
        self->flushPartWordBuffer();
      }
      i++;
