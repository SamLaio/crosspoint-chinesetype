...
XTEink X4 電子紙閱讀器韌體，基於 CrossPoint Reader 修改。這個版本以繁體中文閱讀體驗為核心，整合 EPUB/TXT/Markdown/XTC/XTCH/圖片閱讀、SD 卡檔案管理、OPDS 下載、Wi-Fi 傳書、Calibre 無線傳書、藍牙 HID 翻頁、KOReader Sync、自訂字型、Web 設定，以及 Noto Sans CJK 內建字集。
...
| Build | Version |
| --- | --- |
| 開發版 `default` | `zhTW_V2.5.6-allocate` |
| Release `gh_release` | `zhTW_V2.5.6` |
| RC `gh_release_rc` | `zhTW_V2.5.6-rc+<hash>` |
| Slim `slim` | `zhTW_V2.5.6-slim` |

目前尚未針對 `zhTW_V2.5.6` 重新編譯。工作目錄內既有 `default` 產物只可作為既有產物參考，不代表本版精確大小：

| 項目 | 大小 |
| --- | --- |
| `.pio/build/default/firmware.bin` | `4,607,040 bytes`，約 `4.39 MiB` |
| OTA app partition | `0x640000 = 6,553,600 bytes`，約 `6.25 MiB` |
| 剩餘空間 | 約 `1,946,560 bytes`，約 `1.86 MiB` |

PlatformIO 顯示的 Flash usage 和 `firmware.bin` 檔案大小不同是正常的；前者是 linker 計算的程式區使用量，後者是實際燒錄映像檔。更新字型或程式後仍需重新編譯才能得到最新精確數值。
...
| EPUB 閱讀 | 可用 | 章節、進度跳轉、直排/橫排、每本書版面設定、快取、KOReader Sync |
...
| 內建中文字型 | 可用 | 書本與介面目前統一使用 10px Noto Sans CJK TC 子集字型 |
...
EPUB 閱讀支援章節目錄、閱讀方向、直達進度百分比、KOReader 進度同步、快取清理，以及書本排版/閱讀設定選擇。若書籍內嵌樣式與目前閱讀設定的直排/橫排不同，第一次開啟會跳出選擇畫面；選擇會記錄在該 EPUB 的 cache 目錄，下次開同一本書不再重問。閱讀選單也提供 `版面設定`，可在 `書本排版` 與 `閱讀設定` 間切換，離開選單後才重排並覆寫該書設定。清除 EPUB cache 時會一併清除這份每本書版面設定。
...
目前所有內建書本字型 ID、UI 字型與小字型都指向同一份 Noto Sans CJK TC Regular 子集：

```text
lib/EpdFont/builtinFonts/noto_sans_cjk_18_regular.h
lib/EpdFont/builtinFonts/source/NotoSansCJK/NotoSansCJKtc-Regular.otf
```

這份字型 header 是 10px、2-bit bitmap 字型。現階段字集以手動產生並提交為準，`platformio.ini` 不會在 build 前自動重建字集或字型 header。

目前字集來源包含：

- 常用繁體中文、簡體中文與英文
- `LanguageMapper` 內的繁中、簡中、英文介面文字
- 教育部次常用字
- 常用符號、全形/半形符號、直排標點與破折號變體

目前 Noto Sans CJK TC 子集約為 `12,840 glyphs`、`4,616 intervals`、`1,266,478 bitmap bytes`，header 文字檔約 `8.3M`。這不是完整 CJK 全字庫，而是為韌體大小取捨後的常用字級別字集。

如需手動重建字集與字型 header，可依序執行：

```bash
python3 scripts/generate_ui_charset.py
cd lib/EpdFont/scripts
/home/sam/.venvs/crosspoint-chinesetype/bin/python fontconvert.py noto_sans_cjk_18_regular 10 ../builtinFonts/source/NotoSansCJK/NotoSansCJKtc-Regular.otf --2bit --charset-file ../../../tools/font-subset/ui_charset.txt > ../builtinFonts/noto_sans_cjk_18_regular.h
...
目前內建字型已統一到 Noto Sans CJK TC 子集。設定頁仍會保留 Bookerly、NotoSans、OpenDyslexic 等字型 ID/選項，但在這個版本中，這些內建 ID 都指向同一份 `noto_sans_cjk_18_regular` 資料。若需要真正不同字體風格，請使用 SD 卡自訂 `.epdfont` 字型。
...
需要 PlatformIO。WSL 內已有 `pio` 時可直接使用：
